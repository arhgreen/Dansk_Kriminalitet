      ...
      borderLeft: '2px solid #2db5ff' }
                       ...
                       columns = list(Antal = colDef(aggregate = "sum",
                                      ...
                                      Region = colDef(searchable = TRUE,
                                                      filterable = TRUE))) %>% 
  add_title(title = "Antal personer anmälda och åtalade för brott per händelse, region",
            font_color = "black",
            font_size = 18) %>% 
  add_subtitle(subtitle = "mellan 2015-2024",
               font_weight = "normal",
               font_size = 14)
...
```{r sexualbrott, fig.height=6, fig.width=12.5}
# Alla sexualbrott
...
anmälda_åtalade_region %>% 
  ...
  ggplot(aes(x = År,
              y = Antal,
              group = 1)) +
  geom_point_interactive(aes(tooltip=År),size = 1,
             colour = "dodgerblue") +
